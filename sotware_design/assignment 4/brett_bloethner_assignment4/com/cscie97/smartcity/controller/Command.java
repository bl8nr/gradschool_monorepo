...
public abstract class Command {
    ...
    SmartCityAuthenticationService smartCityAuthenticationService;
    City city = null;
    Device device = null;
    Person subject = null;
     ...
     */
    public Command(SmartCityModelService smartCityModelService, SmartCityLedgerService smartCityLedgerService, String cityId, String deviceId, String subjectId, String authToken) throws CommandException {
        this.smartCityModelService = smartCityModelService;
        this.smartCityLedgerService = smartCityLedgerService;
        this.smartCityAuthenticationService = SmartCityAuthenticationService.getInstance();
        this.authToken = authToken;

        // if the cityId is provided then get the city, otherwise leave it null
        if (cityId != null) {
            try {
                this.city = smartCityModelService.getCityById(cityId, authToken);
        ...
        if (subjectId != null) {
            try {
                this.subject = this.smartCityModelService.showPerson(subjectId, this.authToken);
            } catch (SmartCityModelServiceException e) {
                throw new CommandException("Failed to showPerson when subjectId was provided");
            }
        }

        System.out.printf("Controller Command created with Auth Token %s\n", this.authToken);
    }
    ...
    public void respondFromSensingDeviceSpeaker(String message) throws CommandException {
        ...
        try {
            ...
            this.smartCityModelService.createSensorOutput(this.city.getCityId(), this.device.getDeviceId(), sensorOutput, this.authToken);
        } catch (SmartCityModelServiceException e) {
    ...
    public void mobilizeNearestAvailableRobot(String activity) throws CommandException {
        ...
        try {
            ...
            this.smartCityModelService.updateDevice(this.city.getCityId(), responder.getDeviceId(), responder, this.authToken);
