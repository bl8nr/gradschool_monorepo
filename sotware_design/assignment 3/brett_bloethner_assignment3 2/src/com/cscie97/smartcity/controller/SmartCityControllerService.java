...
public class SmartCityControllerService implements iObserver {
     ...
     */
    private SmartCityControllerService(SmartCityModelService smartCityModelService, SmartCityLedgerService smartCityLedgerService) {
    ...
    public static SmartCityControllerService getInstance() {
        ...
        if (uniqueInstance == null) {
            uniqueInstance = new SmartCityControllerService(SmartCityModelService.getInstance(), SmartCityLedgerService.getInstance());
        ...
        switch (sensorEvent.getType()) {
            ...
            case CAMERA:
                switch (sensorEvent.getValue().toLowerCase()) {
                    ...
                    case "fire":
                        command = new EmergencyOne(smartCityModelService, smartCityLedgerService, EmergencyOneTypeEnum.FIRE, sensorEvent.getCityId(), sensorEvent.getDeviceId());
                        break;
                    // and it sensed a flood, then create an emergency one command
                    case "flood":
                        command = new EmergencyOne(smartCityModelService, smartCityLedgerService, EmergencyOneTypeEnum.FLOOD, sensorEvent.getCityId(), sensorEvent.getDeviceId());
                        break;
                    // and it sensed a earthquake, then create an emergency one command
                    case "earthquake":
                        command = new EmergencyOne(smartCityModelService, smartCityLedgerService, EmergencyOneTypeEnum.EARTHQUAKE, sensorEvent.getCityId(), sensorEvent.getDeviceId());
                        break;
                    // and it sensed a severe weather event, then create an emergency one command
                    case "severe weather":
                        command = new EmergencyOne(smartCityModelService, smartCityLedgerService, EmergencyOneTypeEnum.SEVERE_WEATHER, sensorEvent.getCityId(), sensorEvent.getDeviceId());
                        break;
                    // and it sensed a traffic accident event, then create an emergency one command
                    case "traffic_accident":
                        command = new EmergencyTwo(smartCityModelService, smartCityLedgerService, EmergencyTwoTypeEnum.TRAFFIC_ACCIDENT, sensorEvent.getCityId(), sensorEvent.getDeviceId());
                        break;
                    // and it sensed a littering event, then create a litter event command
                    case "littering":
                        command = new LitterEvent(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject());
                        break;
                    // and it sensed a person seen event, then create a person seen command
                    case "person_seen":
                        command = new PersonSeen(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject());
                        break;
                    // and it sensed a person boarding bus event, then create a board bus command
                    case "person boards bus":
                        command = new BoardBus(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject());
                        break;
                    default:
                        ...
                        if (sensorEvent.getValue().toLowerCase().contains("parked for")) {
                            ...
                            command = new ParkingEvent(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), cityOfVehicle, idOfVehicle, duration);
                            break;
                        } else {
            ...
            case MICROPHONE:
                ...
                if (sensorEvent.getValue().equals("broken_glass_sound")) {
                    command = new BrokenGlass(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId());
                // else if the value implies a bus route inquiry event, then create a bus route command
                    ...
                    command = new BusRoute(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject(), sensorEvent.getValue().substring(20, (sensorEvent.getValue().length() - 1)));
                // else if the value implies a missing child event, then create a missing command
                    ...
                    command = new MissingChild(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject());
                // else if the value implies a movie info inquiry event, then create a movie info command
                    ...
                    command = new MovieInfo(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject());
                // else if the value implies a movie reservation event, then create a movie reservation command
                    ...
                    command = new MovieReservation(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), sensorEvent.getSubject());
            ...
            case CO2METER:
                ...
                if (currentCo2Record >= 3) {
                    command = new CO2Event(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(),false);
                // else if the resulting city CO2 record is -3 or less, then create CO2 command to enable all cars in city
                    ...
                    command = new CO2Event(smartCityModelService, smartCityLedgerService, sensorEvent.getCityId(), sensorEvent.getDeviceId(), true);
                }
                break;
